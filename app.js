  ...
  app.get("/robots.txt", function(req,res) {
    ...
    res.send(`Sitemap: https://op.xyz/sitemap.xml`);
});

app.get("/sitemap.xml", async function(req,res) {
    let link = "<url><loc>https://op.xyz/</loc></url>";
    let botdataforxml = await botsdata.find()
    botdataforxml.forEach(bot => {
        link += "\n<url><loc>https://op.xyz/bot/"+bot.botID+"</loc></url>";
    })
  ...
  const renderTemplate = (res, req, template, data = {}) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://op.xyz/user/${a.id}) user ** tried to login to the site ** but could not login because it was blocked from the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "You have been blocked from OP.", reason: `${banned.sebep}` })
        req.logout();
        });
        } else {
    ...
    app.post("/bot/:botID/edit", checkMaintence, checkAuth, async (req,res) => {
      ...
      app.post("/certification/apply", checkMaintence, checkAuth, async (req, res) => {
          ...
          client.channels.cache.get(channels.botlog).send(`User **${botdata.ownerName}** requested a certificate for bot **${botdata.username}**.`)
      });
      //
      ...
      app.get("/admin/confirm/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         client.users.fetch(req.params.botID).then(bot => {
              client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> bot **${bot.tag}** has been approved. `)
              client.users.cache.get(botdata.ownerID).send(`Your bot **${bot.tag}** has been approved.`)
         });
     ...
     let botdata = await botsdata.findOne({ botID: req.params.botID });
           client.users.fetch(botdata.ownerID).then(sahip => {
               client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> bot **${botdata.username}** has been declined. `)
               client.users.cache.get(botdata.ownerID).send(`Your bot **${botdata.username}** has been declined.\nReason: **${rBody['reason']}**\nAuthorized: **${req.user.username}**`)
          })
          await botsdata.deleteOne({ botID: req.params.botID, ownerID: botdata.ownerID })
       ...
       app.get("/admin/certificate/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         let botdata = await botsdata.findOne({ botID: req.params.botID });
          ...
          client.users.fetch(botdata.botID).then(bot => {
              client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> bot **${bot.tag}** has been granted a certificate.`)
              client.users.cache.get(botdata.ownerID).send(`Your bot **${bot.tag}** has been certified.`)
          });
       ...
       app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         let botdata = await botsdata.findOne({ botID: req.params.botID });
          client.users.fetch(botdata.botID).then(bot => {
              client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> bot **${bot.tag}** has not been granted a certificate.`)
              client.users.cache.get(botdata.ownerID).send(`Your bot **${bot.tag}** certificate application has been declined.\nReason: **${rBody['reason']}**`)
          });
          await appsdata.deleteOne({ botID: req.params.botID })
          let guild = client.guilds.cache.get(settingsc.serverID)
      ...
      app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`<a:maintence:833375738547535913> OP has been switched to __maintance__ due to **${req.body.reason}**`).then(a => { 
      ...
      app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        .setAuthor("OP.xyz", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<a:online:833375738785824788> OP are **active** again!\n[Click to redirect website](https://OP.xyz)`)
        .setFooter("OP © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => { a.edit(`~~ <a:maintence:833375738547535913> OP has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesvcodes) } )
